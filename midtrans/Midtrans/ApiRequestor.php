...
class ApiRequestor
{
    ...
    public static function remoteCall($url, $server_key, $data_hash, $method)
    {
        ...
        if (Config::$paymentIdempotencyKey)
            Config::$curlOptions[CURLOPT_HTTPHEADER][] = 'Idempotency-Key: ' . Config::$paymentIdempotencyKey;

        // merging with Config::$curlOptions
        if (count(Config::$curlOptions)) {
            // We need to combine headers manually, because it's array and it will no be merged
            if (isset(Config::$curlOptions[CURLOPT_HTTPHEADER])) {
                $curl_options[CURLOPT_HTTPHEADER] = array_merge($curl_options[CURLOPT_HTTPHEADER], Config::$curlOptions[CURLOPT_HTTPHEADER]);
            }
            $curl_options = array_replace_recursive($curl_options, Config::$curlOptions);
        }
